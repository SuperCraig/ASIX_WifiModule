...
s16              gudpbc_SocketFD_MBI;            //Craig 171106
struct sockaddr_in	gudpbc_DestAddr_MBI;    //Craig 171106
...
static void gudpbc_ConfigHandle(void *param)
{
	...
	{          
        ...
        if (recvLen != 0){ //Ian test modify; check udp length
            ...
            printf("\n\r UDP_receive: SrcPort = %x ", gudpbc_DestAddr.sin_port);  
            printf("\n\r UDP_receive: DataPacket= ");         //Craig 171030
            for(i=0;i<recvLen;i++)
              printf("%x ",*(pBcRcvrBuf+i));
            
            gudpbc_DestAddr.sin_port=0x2262;            //set the port 25122
            char *string="Zac is handsome!";
            
            sendto(gudpbc_SocketFD, &string[0], 16, 0, &gudpbc_DestAddr, sizeof(struct sockaddr_in));           // Craig
            sendto(gudpbc_SocketFD, &Co2_1[0], 18, 0, &gudpbc_DestAddr, sizeof(struct sockaddr_in));           // Craig
            sendto(gudpbc_SocketFD, &PM_1[0], 17, 0, &gudpbc_DestAddr, sizeof(struct sockaddr_in));           // Craig
            sendto(gudpbc_SocketFD, &PM_2[0], 17, 0, &gudpbc_DestAddr, sizeof(struct sockaddr_in));           // Craig
            sendto(gudpbc_SocketFD, &PM_3[0], 17, 0, &gudpbc_DestAddr, sizeof(struct sockaddr_in));           // Craig
            sendto(gudpbc_SocketFD, &Temp_Hum_1[0], 19, 0, &gudpbc_DestAddr, sizeof(struct sockaddr_in));           // Craig
            sendto(gudpbc_SocketFD, &Temp_Hum_2[0], 19, 0, &gudpbc_DestAddr, sizeof(struct sockaddr_in));           // Craig
        }
        ...
        //if ((*addr&0xff000000) == 0xff000000) {//Ian test modify only broadcast active
            if ((recvLen > 40)&&(memcmp(&GCONFIG_Gid, pBcRcvrBuf, sizeof(GCONFIG_GID)) == 0)){
                ...
                gudpbc_SocketFD_MBI = gudpbc_SocketFD;            //Craig 171106
                
                recvfrom(gudpbc_SocketFD, pBcRcvrBuf, sizeof(GCONFIG_CFG_PKT), 0,                           //Craig 171106
					   (struct sockaddr *)&gudpbc_DestAddr_MBI, (socklen_t *)&iAddrSize);
