...
static int readPacket(MQTTClient* c, Timer* timer)
{
	...
	{
//		printf("\n\rreadPacket(): mqttread(header) failed or nothing\n\r");
        goto exit;
	}
    ...
    {
        printf("\n\rreadPacket(): rem_len = %d, read buffer will overflow", rem_len);
        rc = BUFFER_OVERFLOW;
        goto exit;
    }
...
int MQTTConnect(MQTTClient* c, MQTTPacket_connectData* options)
{
    Timer connect_timer;
    int rc = FAILURE;
    MQTTPacket_connectData default_options = MQTTPacket_connectData_initializer_v4;
    int len = 0;
...
MQTTPublish_Retry:
	...
	{
		printf("\n\rMQTTPublish():sendPacket() failed\n\r");
//		rc = FAILURE;
		rc = TX_FAILURE;
		goto exit; // there was a problem
	}
	...
	if (message->qos == QOS1)
	{
		if (waitfor(c, PUBACK, &timer) == PUBACK)
		{
			if (MQTTDeserialize_ack(&type, &dup, &mypacketid, c->readbuf, c->readbuf_size) != 1)
			{
				printf("\n\rMQTTPublish():MQTTDeserialize_ack(QOS1) failed\n\r");
				rc = FAILURE;
			}
		}
		else
		{
			printf("\n\rMQTTPublish():did not recv PUBACK, ");
//			rc = FAILURE;
			if (++retry_cnt <= 4)
			{
				printf("do retry\n\r");
	...
	else if (message->qos == QOS2)
	{
		if (waitfor(c, PUBCOMP, &timer) == PUBCOMP)
		{
			if (MQTTDeserialize_ack(&type, &dup, &mypacketid, c->readbuf, c->readbuf_size) != 1)
			{
				printf("\n\rMQTTPublish():MQTTDeserialize_ack(QOS2) failed\n\r");
				rc = FAILURE;
			}
		}
		else
		{
			printf("\n\rMQTTPublish():is not PUBCOMP\n\r");
			rc = FAILURE;
		}
	}
