...
uint8_t serial_raed_mcr(serial_t *obj) 
{
    PHAL_RUART_ADAPTER pHalRuartAdapter;
    uint8_t RegValue;
    
    pHalRuartAdapter = &(obj->hal_uart_adp);
    RegValue = HAL_RUART_READ8(pHalRuartAdapter->UartIndex, RUART_MODEM_CTL_REG_OFF);
    return RegValue;
}
...
u8 serial_getUartIndex(serial_t *obj) 
{
    PHAL_RUART_ADAPTER pHalRuartAdapter=(PHAL_RUART_ADAPTER)&(obj->hal_uart_adp);
    return pHalRuartAdapter->UartIndex;
}

void serial_txFifoEnable(PHAL_RUART_ADAPTER pHalRuartAdapter) 
{
    if (serial_irq_en[pHalRuartAdapter->UartIndex] & SERIAL_TX_IRQ_EN) {
        // UnMask TX FIFO empty IRQ
        pHalRuartAdapter->Interrupts |= RUART_IER_ETBEI;
        HalRuartSetIMRRtl8195a (pHalRuartAdapter);
    }
