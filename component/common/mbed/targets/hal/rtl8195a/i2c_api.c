...
static xSemaphoreHandle i2cMstMutex_0;
...
void i2c_sem_init(i2c_t *obj, PinName sda, PinName scl, int freq)
{
  	i2cMstMutex_0 = xSemaphoreCreateMutex();
	i2c_init(obj, sda, scl);
	i2c_frequency(obj, freq);
}
  
u8 i2c_sem_read(i2c_t *obj, int address, int regaddr, char *data, int length, u32 opt)
{
	int ret;
	...
	pi2ccfg->I2CExd = opt;
    pi2ccfg->pRXBuf->RegAddr = regaddr;
	ret = i2c_read(obj, address, data, length, 1);
	xSemaphoreGive(i2cMstMutex_0);
	return ((ret==length) ? TRUE:FALSE);
}
  
u8 i2c_sem_write(i2c_t *obj, int address, int regaddr, const char *data, int length, u32 opt)
{
  	int ret;
	PSAL_I2C_HND pi2ccfg;
	...
	pi2ccfg = (PSAL_I2C_HND)&(obj->SalI2CMngtAdpt.pSalHndPriv->SalI2CHndPriv);
	pi2ccfg->I2CExd = opt;
    pi2ccfg->pTXBuf->RegAddr = regaddr;
	ret = i2c_write(obj, address, data, length, 1);
	xSemaphoreGive(i2cMstMutex_0);
	return ((ret==length) ? TRUE:FALSE);
}
