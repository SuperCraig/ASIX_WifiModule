...
extern char uart_buf[LOG_SERVICE_BUFLEN];
...
#else
   strncpy(uart_buf, log_buf, (LOG_SERVICE_BUFLEN-1));//uart_buf[LOG_SERVICE_BUFLEN]
...
extern int interactive_mode(void *param);
...
#if CONFIG_WLAN
			if(interactive_mode((char *)log_buf) < 0)
#endif                        
			{
