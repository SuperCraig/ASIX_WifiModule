...
static uint8_t dhcp_option_lease_time[4] = {0x00, 0x01, 0x51, 0x80};
...
{	
	...
	if (sender_port == DHCP_CLIENT_PORT) {
		...
		if (udp_packet_buffer->next != NULL) {
			...
			/* FIXED: udp_packet_buffer is freed in pbuf_coalesce. */
...
{
	struct ip_addr addr_pool_start, addr_pool_end;
	u32 lease;
//	printf("dhcps_init,wlan:%c\n\r",pnetif->name[1]);
	...
	addr_pool_start.addr = ntohl(GCONFIG_GetDhcpSrvStartIp());
	addr_pool_end.addr = ntohl(GCONFIG_GetDhcpSrvEndIp());
	dhcps_set_addr_pool(1, &addr_pool_start, &addr_pool_end);
	lease = GCONFIG_GetDhcpSrvLeaseTime() * 60;
	dhcp_option_lease_time[0] = (lease & 0xFF000000) >> 24;
	dhcp_option_lease_time[1] = (lease & 0x00FF0000) >> 16;
	dhcp_option_lease_time[2] = (lease & 0x0000FF00) >> 8;
	dhcp_option_lease_time[3] = (lease & 0x000000FF);
#if 0	
